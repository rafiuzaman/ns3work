...
# ==========================================================
# run_experiments.sh - OLSR Paper 1
#
# Directory structure:
# OLSR_Experiments/
# ├── extract_metrics.awk
...
if [[ "$RATE" != "64" && "$RATE" != "128" ]]; then
    echo "Usage: ./run_experiments.sh 64"
    echo "   or: ./run_experiments.sh 128"
    exit 1
fi
...
OUTPUT_CSV="results/OLSR_final_results_${RATE}kbps.csv"
...
do
  ...
  do
    ...
    do
      ...
      TRACE="$(pwd)/traces/OLSR_${model}_${nodes}nodes_seed${seed}.tr"

      echo "--------------------------------------------"
      echo "Run $CURRENT_RUN / $TOTAL_RUNS"
      echo "Protocol=OLSR | Rate=${RATE}kbps | Model=$model | Nodes=$nodes | Seed=$seed"

      ns mobility.tcl "$seed" "$model" "$nodes" "$TRACE" "${RATE}kb"
      ...
      awk -v simtime=1000 -f extract_metrics.awk "$TRACE" > temp.txt
    ...
    calc_stats() {
      ...
      # Population SD: same convention as the supplied AODV script.
    ...
    echo "Completed OLSR | $model | $nodes nodes"
...
echo "========================================="
echo "All OLSR experiments completed."
