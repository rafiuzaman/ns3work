...
static double g_trustMinSample = 10.0;  // evidence needed before acting
  ...
  private:
    ...
    void HandleAck(Ptr<Socket> socket)
    {
        ...
        while ((packet = socket->RecvFrom(from)))
        {
            ...
            back->Connect(InetSocketAddress(tag.GetSrcAddr(), GW_ACK_PORT));
            back->Send(packet);
        }
    }
...
class NodeAgent : public Application
{
  ...
  private:
    ...
    struct TrustRecord
    {
        ...
        {
            return condemnations ? g_trustMinSample / 4.0 : g_trustMinSample;
        }
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("trustDecay", "Rate at which old delivery evidence ages out",
                 g_trustDecay);
    cmd.AddValue("trustMinSample", "Evidence required before trust is acted upon",
                 g_trustMinSample);
